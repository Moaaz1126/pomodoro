...
function playPause() {
    ...
    if(playPauseEle.text() == "Play") {
        timer();
        x = 0;
        playPauseEle.text("Pause")
    } else {
        x = 120;
        $(".timerp").text("00:00")
        playPauseEle.text("Play")
        setTimeout(function() { 
            $(".timerp").text("00:00")
        }, 1000)
    }
...
function timer() {
    let timeing = 0;
    if(menuStat == 0) {
        min = window.localStorage.getItem("promo");
        soc =  0;
        timeing = min
    }
    if(menuStat == 1) {
        min = window.localStorage.getItem("short");
        soc =  0;
        timeing = min
    }
    if(menuStat == 2) {
        min = window.localStorage.getItem("long");
        soc =  0;
        timeing = min
    }
    let soc2 = 0;
    ...
    function myfor() {
        // if(x != 120) {
            ...
            timerSpan(min, timeing)
            }, 100)
        // }
    }
}
function timerSpan(TheTime, AllTime) {
    let handred = 100 / AllTime
    let insco = handred / 60
    let m = 0;
    let mySpan = document.querySelector(".appTimeSpan")
    let pares =  ( (TheTime / AllTime) * 100 )
    console.log(pares)
    let pares2 = pares + "%"
    root.style.setProperty("--height", pares2)
